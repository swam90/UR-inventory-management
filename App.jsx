...
import {
  ...
  StickyNote, Building2, Boxes, Receipt, Search, Cloud, CloudOff,
  RefreshCw, CheckCircle2, AlertCircle, Server
} from 'lucide-react';
...
/* NAS SYNC
   The NAS runs a tiny PHP endpoint (api.php) that stores the whole
   app dataset as one JSON document, protected by a secret API key.
   This app pushes the full dataset after every change (debounced),
   and pulls the latest copy on startup / manual sync. */
const loadSyncConfig = () => {
  try {
    const raw = localStorage.getItem('inv:syncConfig');
    return raw ? JSON.parse(raw) : { url: '', apiKey: '' };
  } catch { return { url: '', apiKey: '' }; }
};
const saveSyncConfig = (cfg) => {
  try { localStorage.setItem('inv:syncConfig', JSON.stringify(cfg)); } catch {}
};

const nasFetch = async (cfg, method, body) => {
  if (!cfg.url) throw new Error('No NAS URL configured');
  ...
  if (!res.ok) {
    ...
    throw new Error(`NAS responded ${res.status}: ${text.slice(0, 120)}`);
  }
  return res.json();
};

/* STORAGE (localStorage + NAS sync) */
const useStore = () => {
  ...
  const latestData = useRef({ vendors: [], items: [], notes: '', entries: [] });

  useEffect(() => {
    latestData.current = { vendors, items, notes, entries };
  ...
  useEffect(() => {
    ...
    if (v) setVendorsState(v);
    if (i) setItemsState(i);
    if (n !== null) setNotesState(n);
    if (e) setEntriesState(e);
    setReady(true);

    const cfg = loadSyncConfig();
    if (cfg.url && cfg.apiKey) {
      pullFromNas(cfg, { silent: true });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const pullFromNas = async (cfgOverride, opts = {}) => {
    const cfg = cfgOverride || syncConfig;
    if (!cfg.url || !cfg.apiKey) {
    ...
    setSyncError('');
    try {
      const data = await nasFetch(cfg, 'GET');
      if (data && typeof data === 'object') {
        if (Array.isArray(data.vendors)) { setVendorsState(data.vendors); persistLocal('inv:vendors', data.vendors); }
        if (Array.isArray(data.items))   { setItemsState(data.items);     persistLocal('inv:items', data.items); }
        if (typeof data.notes === 'string') { setNotesState(data.notes); persistLocal('inv:notes', data.notes); }
        if (Array.isArray(data.entries)) { setEntriesState(data.entries); persistLocal('inv:entries', data.entries); }
      }
      setSyncStatus('synced');
      setLastSyncedAt(new Date());
    } catch (err) {
      console.error('NAS pull failed', err);
      setSyncStatus(opts.silent ? 'offline' : 'error');
      setSyncError(err.message || 'Could not reach NAS');
    }
  };

  const pushToNas = useCallback(() => {
    const cfg = loadSyncConfig();
    ...
    pushTimer.current = setTimeout(async () => {
      ...
      setSyncError('');
      try {
        await nasFetch(cfg, 'POST', latestData.current);
        setSyncStatus('synced');
        setLastSyncedAt(new Date());
      } catch (err) {
        console.error('NAS push failed', err);
  ...
  const setVendors = (v) => { setVendorsState(v); persistLocal('inv:vendors', v); pushToNas(); };
  const setItems   = (v) => { setItemsState(v);   persistLocal('inv:items', v);   pushToNas(); };
  const setNotes   = (v) => { setNotesState(v);   persistLocal('inv:notes', v);   pushToNas(); };
  const setEntries = (v) => { setEntriesState(v); persistLocal('inv:entries', v); pushToNas(); };
  ...
  return {
    ready,
    vendors, items, notes, entries,
    setVendors, setItems, setNotes, setEntries,
  ...
  return (
    <div style={{ minHeight: '100vh', paddingBottom: 'calc(80px + env(safe-area-inset-bottom))', maxWidth: 1100, margin: '0 auto' }}>
      <header className="no-print" style={{ padding: '20px 18px 8px', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        ...
        <SyncBadge status={store.syncStatus} onTap={() => setTab('sync')} />
      </header>
      ...
      <main style={{ padding: '12px 18px' }}>
        ...
        {tab === 'sync'    && <SyncSettings store={store} />}
      </main>
...
/* SYNC BADGE (header) */
function SyncBadge({ status, onTap }) {
  const map = {
    unconfigured: { icon: CloudOff, color: 'var(--muted)', label: 'Not synced' },
    syncing:      { icon: RefreshCw, color: 'var(--amber)', label: 'Syncing…' },
    synced:       { icon: Cloud, color: 'var(--moss)', label: 'Synced' },
    offline:      { icon: CloudOff, color: 'var(--wine)', label: 'Offline' },
    error:        { icon: AlertCircle, color: 'var(--wine)', label: 'Sync error' },
  };
  ...
  return (
    <button onClick={onTap} title={m.label} style={{
      ...
      color: m.color, padding: 4,
    }}>
      <Icon size={18} className={status === 'syncing' ? 'spin' : ''} />
      <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em' }}>{m.label}</span>
    </button>
  );
}
      ...
      {items.map(({ k, icon: Icon, label }) => {
        ...
        return (
          <button key={k} onClick={() => setTab(k)} style={{
            background: 'transparent', border: 'none', cursor: 'pointer',
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2,
            padding: '6px 10px', color: active ? 'var(--moss)' : 'var(--muted)',
...
function Dashboard({ store, go }) {
  ...
  return (
    <div>
      {store.syncStatus === 'unconfigured' && (
        <button onClick={() => go('sync')} className="card" style={{
          width: '100%', padding: '12px 14px', marginBottom: 14, textAlign: 'left',
          display: 'flex', alignItems: 'center', gap: 10, background: '#fff8ec', borderColor: 'var(--amber)'
        }}>
          <CloudOff size={18} color="var(--amber)" />
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: 13 }}>Set up NAS sync</div>
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>Back up and share data across devices</div>
          </div>
        </button>
      )}
...
/* SYNC SETTINGS TAB */
function SyncSettings({ store }) {
  const [url, setUrl] = useState(store.syncConfig.url || '');
  const [apiKey, setApiKey] = useState(store.syncConfig.apiKey || '');
  const [saved, setSaved] = useState(false);

  const statusMap = {
    unconfigured: { icon: CloudOff, color: 'var(--muted)', text: 'Not set up yet' },
    syncing:      { icon: RefreshCw, color: 'var(--amber)', text: 'Syncing…' },
    synced:       { icon: CheckCircle2, color: 'var(--moss)', text: 'Up to date' },
    offline:      { icon: CloudOff, color: 'var(--wine)', text: 'Could not reach NAS' },
    error:        { icon: AlertCircle, color: 'var(--wine)', text: 'Sync error' },
  };
  const s = statusMap[store.syncStatus] || statusMap.unconfigured;
  const SIcon = s.icon;

  const save = () => {
    store.updateSyncConfig({ url: url.trim(), apiKey: apiKey.trim() });
    setSaved(true);
  ...
  return (
    <div>
      <h2 className="serif" style={{ fontSize: 20, fontWeight: 700, margin: '0 0 4px' }}>NAS Sync</h2>
      <p style={{ fontSize: 13, color: 'var(--muted)', margin: '0 0 16px' }}>
        Keep your data backed up and in sync across every device via your own Synology NAS.
      </p>

      <div className="card" style={{ padding: 14, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
        <SIcon size={22} color={s.color} className={store.syncStatus === 'syncing' ? 'spin' : ''} />
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 700, fontSize: 14, color: s.color }}>{s.text}</div>
          {store.lastSyncedAt && (
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>
              Last synced {store.lastSyncedAt.toLocaleTimeString('en-SG', { hour: '2-digit', minute: '2-digit' })}
            </div>
          )}
          {store.syncError && (
            <div style={{ fontSize: 12, color: 'var(--wine)', marginTop: 2 }}>{store.syncError}</div>
          )}
        </div>
        {store.syncConfig.url && (
          <button className="btn btn-ghost" onClick={store.syncNow} style={{ padding: '6px 10px', fontSize: 12 }}>
      ...
      <div className="card" style={{ padding: 16, display: 'grid', gap: 14 }}>
        <div>
          <label>NAS sync URL</label>
          <input
            value={url}
            onChange={e => setUrl(e.target.value)}
            placeholder="https://yourname.synology.me/inventory/api.php"
            inputMode="url"
            autoCapitalize="none"
            autoCorrect="off"
          />
        ...
        <div>
          ...
          <input
            ...
            placeholder="The API_KEY you set in api.php"
            type="password"
            autoCapitalize="none"
            autoCorrect="off"
          />
        ...
        <button className="btn btn-primary" onClick={save}>
          <Save size={15} /> {saved ? 'Saved ✓' : 'Save & connect'}
        </button>
      </div>

      <div className="card" style={{ padding: 16, marginTop: 16, background: 'var(--paper-2)', border: 'none' }}>
        <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 6 }}>How this works</div>
        <ul style={{ margin: 0, paddingLeft: 18, fontSize: 12.5, color: 'var(--muted)', lineHeight: 1.6 }}>
          <li>Every change you make is pushed to your NAS a moment later.</li>
          <li>Opening the app on any device pulls the latest data from the NAS.</li>
          <li>If the NAS is unreachable, the app keeps working from its local copy and syncs again once reconnected.</li>
          <li>Your NAS address and secret key are stored only on this device, never uploaded anywhere else.</li>
        </ul>
      </div>
    </div>
  );
